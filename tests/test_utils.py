...
class TestUtils(unittest.TestCase):
    ...
    def test_to_dateroll_date(self):
        ...
        bad_base = 'apple'
        self.assertEqual(utils.to_dateroll_date(good_string_tenor,good_base),good_base+ddh(good_string_tenor))
    ...
    def test_from_date_to_number(self):
        ...
        good_date = ddh('t')
        bad_type = 5
        self.assertEqual(utils.from_date_to_number(good_date, root, "ACT/365", cal),(good_date - root).just_days)
        self.assertEqual(utils.from_date_to_number(good_date, root, "bd/252", cal),(good_date - root).just_days)
        with self.assertRaises(Exception):
    ...
    def test_from_number_to_date(self):
        ...
        number = 10
        dc = "ACT/365"
        self.assertEqual(utils.from_number_to_date(number,root,dc, cal),root + number)
    ...
    def test_delta_t(self):
        ...
        dc = "ACT/365"
        cal = "ALL"
        self.assertEqual(utils.delta_t(date1, date2, dc, cal),(date2 - date1).yf(cal, dc))
